...
func TestIterGrowWithGC_LowLocking(t *testing.T) {
	m := NewLowLockingMap(4) //	make(map[int]int, 4)
	for i := 0; i < 16; i++ {
		m.Put(i, i)
	}
...
func testConcurrentReadsAfterGrowth_LowLocking(t *testing.T, useReflect bool) {
	...
	for i := 0; i < numLoop; i++ {
		m := NewLowLockingMap() //	make(map[int]int, 0)
...
func TestConcurrentReadsAfterGrowth_LowLocking(t *testing.T) {
	testConcurrentReadsAfterGrowth(t, false)
}

func TestConcurrentReadsAfterGrowthReflect_LowLocking(t *testing.T) {
	testConcurrentReadsAfterGrowth(t, true)
}

func TestBigItems_LowLocking(t *testing.T) {
	var key [256]string
	for i := 0; i < 256; i++ {
		key[i] = "foo"
	}
	m := NewLowLockingMap(4) //make(map[[256]string][256]string, 4)
	for i := 0; i < 100; i++ {
...
func TestEmptyKeyAndValue_LowLocking(t *testing.T) {
	...
	a := NewLowLockingMap(4)
	b := NewLowLockingMap(4)
	c := NewLowLockingMap(4)
...
func TestSingleBucketMapStringKeys_DupLen_LowLocking(t *testing.T) {
	testMapLookups_LowLocking(t, NewConcurrentMapFromMap(map[interface{}]interface{}{
		"x":    "x1val",
		"xx":   "x2val",
		"foo":  "fooval",
...
func TestSingleBucketMapStringKeys_NoDupLen_LowLocking(t *testing.T) {
	testMapLookups_LowLocking(t, NewConcurrentMapFromMap(map[interface{}]interface{}{
		"x":                      "x1val",
		"xx":                     "x2val",
		"foo":                    "fooval",
...
func TestMapIterOrder_LowLocking(t *testing.T) {
...
/*----------------test concurrent-------------------------------*/
func TestConcurrent_LowLocking(t *testing.T) {
	...
	for i := 0; i < readN; i++ {
		go func() {
			for {
				itr := NewLowLockingMapIterator(cm)
				for itr.HasNext() {
					entry := itr.NextEntry()
					k := entry.Key().(int)
					v := entry.Value().(string)
